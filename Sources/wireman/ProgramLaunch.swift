...
struct MainProgram {
	static func main() async {
		...
		let whichWireguard = try! await Command(bash:"which wg").runSync().stdout.compactMap { String(data:$0, encoding:.utf8) }.first!
		print("wireguard found at \(whichWireguard)")
		...
		await AsyncGroup {
			...
			$0.command("client_make") {
				...
				let interface = try database.primaryInterfaceName()
				if try await Command(command:"/usr/bin/wg set \(interface) peer \(publicKey) allowed-ips \(useAddress.string)/128 preshared-key \(pskPath.path)")!.runSync().succeeded == false {
					print("!!failed to set new config with /usr/bin/wg")
					exit(15)
				}
				if try await Command(bash:"/usr/bin/wg-quick save \(interface)").runSync().succeeded == false {
